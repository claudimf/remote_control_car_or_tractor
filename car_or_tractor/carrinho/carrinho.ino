...
//int sensorMagnetico = A0;// PINO DO SENSOR MAGNÉTICO É A0
...
//ESCREVER O TEXTO DA SERIAL E DIVIDE EM TIPO STRING OU FLOAT
void output(char *code,float val) {
  //Serial.print(code);
  //Serial.println(val);  
}

//EXIBIR O NOME DO GRUPO NA SERIAL
void help() {
  //Serial.println(F("Grupo "));
}

//PREPARA A ENTRADA PARA RECEBER AS MENSAGENS DA SERIAL
void ready() {
  sofar=0;  // LIMPAR A ENTRADA DO BUFFER
  //Serial.print(F(">"));  // RECEBER SINAL NA ENTRADA
}
...
void setup() {
  ...
  //pinMode(sensorMagnetico, INPUT);//SENSOR MAGNÉTICO
...
void loop() {
  ...
  while(Serial.available() > 0) {  //HAVENDO ALGO A RECEBER
    ...
    //Serial.print(c);  //IMPRIMI A MENSAGEM RECEBIDA
